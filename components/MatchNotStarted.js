...
import { addPendingGamesToStore } from "../reducers/games";

function MatchNotStarted() {
  const dispatch = useDispatch();
  const infoPendingGames = useSelector(
    (state) => state.games.value.pendingGames
  );
  ...
  return <div className={styles.container}>{infoPending}</div>;
}
